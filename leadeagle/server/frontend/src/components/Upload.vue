...
export default {
  ...
  this.dropArea.className = "area drag";
}, 
filesDroped(event) {
  var webkitResult = [];
  var mozResult = [];
  ...
  function listDirectory(entry) {
    ...
    var path =
      entry.fullPath ||
      entry.webkitRelativePath.slice(
        0,
        entry.webkitRelativePath.lastIndexOf("/")
      );
    var cname = path
      .split("/")
      .filter(Boolean)
    ...
    if (curr.length) {
      for (li of curr[curr.length - 1].querySelectorAll("li")) {
        if (li.innerHTML.indexOf(path.slice(0, path.lastIndexOf("/"))) > -1) {
          li.querySelector("span").insertAdjacentHTML(
            "afterend",
            `${filesInfo}`
          );
          appended = true;
...
created() {
this.dropArea.addEventListener("dragover", dragHandler);
this.dropArea.addEventListener("change", filesDroped);
this.dropArea.addEventListener("drop", filesDroped);
}
  }
};
